...
class Parser:
    ...
    def parse(self):
        ...
        def expression_equality(state, p):
            if p[1].gettokentype() == 'EQ':
                if self.syntax is True:
                    return [Node("expression", p[0]), Node("=="), Node("expression", p[2])]
                return Equal(p[0], p[2], state)
            elif p[1].gettokentype() == 'NEQ':
                if self.syntax is True:
                    return [Node("expression", p[0]), Node("!="), Node("expression", p[2])]
                return NotEqual(p[0], p[2], state)
            elif p[1].gettokentype() == 'GTEQ':
                if self.syntax is True:
                    return [Node("expression", p[0]), Node(">="), Node("expression", p[2])]
                return GreaterThanEqual(p[0], p[2], state)
            elif p[1].gettokentype() == 'LTEQ':
                if self.syntax is True:
                    return [Node("expression", p[0]), Node("<="), Node("expression", p[2])]
        ...
        def program(state, p):
            if self.syntax is True:
                return [Node("INPUTTER"), Node("("), Node(")")]
            return Input()

        @self.pg.production('expression : INPUTTER LEFT_PARENT expression RIGHT_PARENT')
        def program(state, p):
            if self.syntax is True:
                return [Node("INPUTTER"), Node("("), Node("expression", p[2]), Node(")")]
